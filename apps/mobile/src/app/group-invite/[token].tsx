...
export default function AcceptGroupInviteScreen() {
  ...
  return (
    <Screen contentContainerStyle={styles.page}>
      ...
      {!groupName ? <Pressable accessibilityRole="button" disabled={!token || accept.isPending} onPress={async () => { if (!token) return; const group = await accept.mutateAsync(token); setGroupName(group.name); }} style={[styles.button, { backgroundColor: theme.primary }]}><Text style={[styles.buttonText, { color: theme.inverse }]}>{accept.isPending ? '참여하는 중…' : '그룹 초대 수락'}</Text></Pressable> : null}
